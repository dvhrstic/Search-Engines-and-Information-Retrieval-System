...
public class Searcher {
    ...
    //public HashMap<Integer, Double> idToScore = new HashMap<Integer, Double>();
    //public HashMap<String,HashMap<Integer, Double>> termToScore= new HashMap<String, HashMap<Integer, Double>>();
...
/*         for (Map.Entry<Integer, Double> rankEntry : pageRanks.entrySet()) {
    ...
    private PostingsList multiRanked(Query query, RankingType rankingType){
        ...
        for (Query.QueryTerm searchQuery : query.queryterm){
            ...
            for (int j = 0; j < postingsCurr.size(); j++){
                ...
                if (!rankedPos.containsDoc(postingsEntry.docID)){
                    ...
                    rankedPos.get(rankedPos.indexOf(postingsEntry.docID)).score += wFtd * searchQuery.weight;
        ...
        for (int k = 0; k < rankedPos.size(); k++){
            ...
            rankedPos.get(k).score = rankedPos.get(k).score  / index.docLengths.get(docID); 
        }
        rankedPos.sort();
        return rankedPos;
    }
